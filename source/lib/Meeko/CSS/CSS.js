...
var Parser = function() {
	this.parser = new CSS.SAC.Parser();
	this.parser.selectorFactory = _selectorFactory;
	this.parser.conditionFactory = _conditionFactory;
}

Parser.prototype.parseSelectors = function(text)
{
	var selectorList = this.parser.parseSelectors(text);
	forEach (selectorList, function(selector) {
		selector.__refresh();
	});
	return selectorList;
}
...
// and receive Selectors unless otherwise specified
	...
	createConditionalSelector: function(s, /* RelativeSelector: */ condition) {
		var r = s.steps[0];
		var r2 = condition;
		forEach (r2.conditions, function(c) {
			r.addCondition(c);
		});
		return s;
	},
	...
	createDescendantSelector: function(s, descendant) {
		return this.createCombinatorialSelector(s, child, RelativeSelector.DESCENDANT_RELATIVE);
	},
